...
// Pagination is a class that represents pagination
...
//constant for upload folder
    ...
    { // actionIndex() is the default action in a controller
    ...
    {
        try {
            $model_student_reset = new StudentResetForm();
            //create an instance of the StudentLoginForm class
            ...
            return $this->render('reset-password',
                ['model_student_reset' => $model_student_reset]);
    ...
    { //action for handling registration form
        try {
            $model_student_register = new StudentRegisterForm();
            //create an instance of the StudentRegisterForm class
            if ($model_student_register->load(Yii::$app->request->post()) &&
                $model_student_register->registerStudent()) {
                //if the form is submitted and the registration is successful
                return $this->redirect(['student/student-token-activate']);
                //go to the next page, customize this to go to the home page
            }
            return $this->render('register-student',
                ['model_student_register' => $model_student_register]);
    ...
    public function actionTokenStudent()
    {
        try {
            ...
            if ($model_student_token->load(Yii::$app->request->post()) &&
                $model_student_token->validateAccessToken()) {
                return $this->goBack(); //go to the previous page, customize this to go to the home page
            }
            return $this->render('token-student',
                ['model_student_token' => $model_student_token]); //render the token student page
        } catch (Exception $e) {
            Yii::$app->session->setFlash('error',
                'An error occurred while processing your request. Please try again later.');
            return $this->redirect(['student/error']);
    ...
    { //action for personal information form
        try {
            ...
            $model_student_data_diri = StudentDataDiriForm::findDataDiri();
            //create an instance of the StudentDataDiriForm class
            ...
            if ($model_student_data_diri->load(Yii::$app->request->post())) {
                if ($model_student_data_diri->insertDataDiri()) {
                    return $this->redirect(['student/student-data-o-tua']);
                    //go to the next page, customize this to go to the home page
                }
            }
    ...
    private function uploadPasPhotoToAws($form, $file)
    {
        ...
        if ($form->$file) { //check if there is a file uploaded
            ...
            try {
                ...
                Yii::$app->session->setFlash('error', 'Something wrong - academic');
                //may be change this to flash error
            }
        }
    }
    ...
    {
        try {
            $model_student_data_o = StudentDataOForm::findDataOTua();
            //create an instance of the StudentDataDiriForm class
            ...
            return $this->render('student-data-o-tua',
                ['model_student_data_o' => $model_student_data_o]);
            //render the parent information page(data orang tua)
        } catch (Exception $e) {
            Yii::$app->session->setFlash('error',
                'An error occurred while processing your request. Please try again later.');
            return $this->redirect(['student/error']);
    ...
    {
        try {
            $model_student_extra = StudentExtraForm::findDataExtra();
            //create an instance of the StudentExtraForm class
        ...
        try {
            ...
            if ($model_student_akademik->load(Yii::$app->request->post())) {
                ...
                foreach ($filesToUpload as $file) {
                    try {
                        $this->uploadToAws($model_student_akademik, $file,
                            $uploadFolderBase, $currentBatch);
                    } catch (Exception $e) {
//                        Yii::$app->session->setFlash('error', );
                        return $this->render('student-akademik',
                            ['model_student_akademik' => $model_student_akademik]);
                    }
                }
                try {
    ...
    private function uploadToAws($form, $file, $uploadFolderBase, $currentBatch)
    {
        ...
        $uploadFolder = ($currentBatch == self::UTBK ? 'sertifikat' : $file);
        ...
        if ($form->$file) { //check if there is a file uploaded
            ...
            try {
                ...
                Yii::$app->session->setFlash('error',
                    "Something wrong while uploading file");
                //may be change this to flash error
            }
        }
    }
    ...
    {
        try {
            $model = new StudentTokenActivate();
            //create an instance of the StudentTokenActivateForm class
            ...
            return $this->render('student-token-activate',
                ['model' => $model]); //render the token activate page
        } catch (Exception $e) {
            Yii::$app->session->setFlash('error',
    ...
    public function actionStudentBahasa()
    {
        try {
            ...
            return $this->render('student-bahasa', ['model' => $model]);
            //render the bahasa page
        } catch (Exception $e) {
            Yii::$app->session->setFlash('error',
                'An error occurred while processing your request. Please try again later.');
            return $this->redirect(['student/error']);
    ...
    {
        try {
            $model = new StudentInformasiForm();
    ...
    public function actionStudentPengumuman(){
        try{
            ...
            if ($model->load(Yii::$app->request->post()) && $model->insertPengumumanData()) {
                return $this->redirect(['student/student-biaya']);
                //to do: change this to pengumuman page
            }
    ...
    public function actionStudentAnnouncement()
    {
        ...
        /*if($model->load(Yii::$app->request->post()) && $model->insertAnnouncementData()){
            return $this->redirect(['student/student-biaya']);
        //to do: change this to announcement page
        }*/
        try {
    ...
    private function uploadFile($form, $file, $uploadFolderBase, $currentBatch)
    {
        ...
        $fileAttribute = 'file_' . $file;
        if ($form->$file) { //check if there is a file uploaded
            $uploadFolder = $uploadFolderBase . ($currentBatch == self::UTBK ? '' : $file . '/');
            //set the upload folder
            if (!file_exists($uploadFolder)) { //not exist, make a new directory
                mkdir($uploadFolder, 0777, true); //make a new directory
            }
            $fileBaseName = Yii::$app->user->identity->username; //set the file base name
            $filePath = $uploadFolder . $fileBaseName . '.' .
                $form->$file->extension; //set the file path
            try {
    ...
    // Replace 'username:password' with your actual username and password
            ...
            if ($response->isOk) {
                ...
                throw new \yii\web\HttpException($response->statusCode, 'Error fetching data from API: ' .
    $response->content);
            }
        } catch (\Exception $e) {
